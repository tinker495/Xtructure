...
def test_at_set_simple_data():
    ...
    data_to_set_scalar = SimpleData(id=jnp.array(100, dtype=jnp.uint32), value=jnp.array(99.9, dtype=jnp.float32))
...
def test_at_set_vector_data():
    original_data = VectorData.default(shape=(3,)) # position and velocity are (3,3) filled with 0.0
...
def test_at_set_nested_data():
    ...
    data_to_set_single_nested = NestedData.default() # scalar id, value, (3,) position, (3,) velocity
    data_to_set_single_nested = data_to_set_single_nested.replace(
        simple=SimpleData(id=jnp.array(10, dtype=jnp.uint32), value=jnp.array(1.1, dtype=jnp.float32)),
        vector=VectorData(position=jnp.ones(3, dtype=jnp.float32), velocity=jnp.ones(3, dtype=jnp.float32) * 2)
    )
    ...
    assert jnp.array_equal(updated_data.vector.position[0], data_to_set_single_nested.vector.position)
    assert jnp.array_equal(updated_data.vector.velocity[0], data_to_set_single_nested.vector.velocity)
