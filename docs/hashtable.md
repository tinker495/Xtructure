...
A bucketed open-addressing hash table optimized for JAX.
...
*   **Bucketed probing**: Uses `slots_per_bucket` (typically small e.g. 2-4) slots per bucket to resolve collisions.
*   **`HashTable.build(dataclass, seed, capacity)`**:
    ...
    *   `capacity`: Desired user capacity. The internal capacity (`_capacity`) is computed as `int(hash_size_multiplier * capacity / slots_per_bucket)`.
